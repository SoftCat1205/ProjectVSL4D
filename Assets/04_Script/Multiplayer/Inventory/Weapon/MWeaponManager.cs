...
public class MWeaponManager : NetworkBehaviour
{
    ...
    public WeaponScriptableObject GetEquipment(ItemCategory itemCategory)
    {
    ...
    public void Activate(NetworkInputData input, Vector2 direction)
    {
        ...
        {
            if (equippedWeapons[ItemCategory.LeftShoulder] != null)
                equippedWeapons[ItemCategory.LeftShoulder].Weapon.Activate(direction);
        }

        if (input.Buttons.IsSet(InputButtons.RightBack))
        {
            if (equippedWeapons[ItemCategory.RightShoulder] != null)
                equippedWeapons[ItemCategory.RightShoulder].Weapon.Activate(direction);
        }
    }
}